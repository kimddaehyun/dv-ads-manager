...
// 검색광고 캠페인별 (sheet3 섹션2): 유형 2개(파워링크 2그룹 / 쇼핑검색광고 1그룹)
const campGroups: CampaignTypeGroup[] = [
  ...
  {
    type: "쇼핑검색광고",
    rows: [{ group: "핵심상품", metrics: M(40000, 920, 1380000, 3600000, 90, 42) }],
  },
];
...
// 검색광고 캠페인별 (sheet3): 11-12 파워링크2행, 13 파워링크소계, 14 쇼핑1행, 15 쇼핑소계, 16 전체합계
const s3 = readText(re, "xl/worksheets/sheet3.xml");
ok(/<c r="B11"[^>]*><is><t[^>]*>파워링크<\/t>/.test(s3), "검색광고 유형 첫행 라벨(B11=파워링크)");
ok(/<c r="C13"[^>]*><is><t[^>]*>소계<\/t>/.test(s3), "검색광고 파워링크 소계행(C13=소계)");
ok(/<c r="D13"[^>]*><v>63000<\/v>/.test(s3), "검색광고 파워링크 소계 노출=25000+38000=63000(D13)");
ok(/<c r="B16"[^>]*><is><t[^>]*>전체 합계<\/t>/.test(s3), "검색광고 전체합계행(B16)");
ok(/<mergeCell ref="B16:C16"\/>/.test(s3), "검색광고 전체합계 병합(B16:C16)");
ok(/<c r="D16"[^>]*><v>103000<\/v>/.test(s3), "검색광고 전체합계 노출=63000+40000=103000(D16)");
