...
- **sticky 표 헤더 스크롤 떨림의 원인 = 조상 popover/모달의 잔류 transform** (2026-07-02 CDP 프레임 캡처로 실측 확정). 진입 애니메이션이 `transform` 키프레임 + `fill: both/forwards`를 쓰면 종료 후에도 identity transform(`matrix(1,...)`)이 computed style에 영구 잔류하고, transform 낀 조상 아래의 `position:sticky`는 컴포지터 보정이 풀려 메인 스레드에서 1프레임 늦게 따라붙는다 → 스크롤 중 헤더가 밀렸다 돌아오는 떨림. **sticky 자손을 품는 컨테이너의 transform 키프레임 애니메이션은 `fill: backwards`로** (`.dvads-multi-popover`·`.dvads-agency-card` 참고). thead 쪽 CSS(top/will-change/border 방식)를 아무리 바꿔도 못 고친다 — 헛수고했던 경로: box-shadow 교체·top:-1px·will-change 승격 모두 무효. 새 오버레이에 sticky 리스트를 넣을 땐 조상 체인의 `animation`/`transform` 잔류 여부부터 확인.
