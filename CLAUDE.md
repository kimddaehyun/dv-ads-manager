...
- **sticky 표 헤더 "위쪽 선 떨림"의 원인 2가지** (2026-07-02 실측 확정 — CDP 프레임 캡처 + 사용자 스크린샷 픽셀 분석): ① **조상 popover/모달의 잔류 transform** — 진입 애니메이션이 `transform` 키프레임 + `fill: both/forwards`면 종료 후 identity transform이 영구 잔류, transform 낀 조상 아래 sticky는 컴포지터 보정이 풀려 스크롤 중 1프레임 지연 떨림. sticky 자손을 품는 컨테이너의 transform 애니메이션은 반드시 `fill: backwards` (`.dvads-multi-popover`·`.dvads-agency-card`). ② **sticky 고정 위치의 sub-pixel 반올림 어긋남** — 스크롤 오프셋이 소수점(125% 배율·스무스 휠)일 때 sticky 헤더 전체가 1px 밀린 프레임이 생기고, 그 틈으로 밑을 지나는 행 테두리(#ECEEF0)가 비쳐 "선이 생겼다 없어짐". thead 쪽 CSS(top/will-change/border 방식)로는 제어 불가 — 해법은 (a) popover 좌표 `Math.round` 정수화(`applyAnchoredPosition`), (b) 스크롤 영역 첫 2px을 헤더 배경색으로 항상 덮는 가림막 `.dvads-multi-grouptabs::after` (탭 바 margin 6px + bottom:-8px 좌표 결합 — 탭 바/여백 수치 바꾸면 같이 갱신). 새 오버레이에 sticky 리스트를 넣을 땐 조상 transform 잔류 + 상단 경계 가림을 함께 챙길 것.
