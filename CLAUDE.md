...
- **`host_permissions` 최소화 방침은 폐기 (2026-08-13 사용자 결정).** 웹스토어가 아니라 GitHub Release zip을 직접 로드하는 사내 전용 배포라 심사가 없다. 새 도메인이 필요하면 그냥 추가한다 — 권한을 아끼려고 우회 설계(서버 대행 등)를 하지 말 것. **단 좁혔다 넓혔다 왕복은 금지** (권한이 늘면 크롬이 확장을 비활성화하고 재승인을 요구할 수 있어, 그때마다 전 사용자가 치른다). 현재 접근처: `ads.naver.com`·`api.searchad.naver.com`(광고 데이터), pstatic CDN 2곳(이미지 binary — CORS 차단이라 background fetch 필수), supabase(AI 중계·인증). F-AutoSetup(보류)이 재개되면 임의 링크를 읽기 위해 `<all_urls>`가 다시 필요하다 — 그때는 **배포 시점에 사용자 재승인이 발생**하므로 다른 변경과 묶어 한 번에 내보낼 것.
- `ads.naver.com` 비공식 internal API(`/apis/sa/api/*`, `/apis/ad-account/v2/*` 등)는 CORS상 **콘텐츠 스크립트에서만 호출 가능** (background는 차단). 인증은 로그인 쿠키 + `x-xsrf-token` 헤더(`XSRF-TOKEN` 쿠키 더블 서밋, `decodeURIComponent` 필요). schema·path 예고 없이 변경 가능 — `friendly-error`로 graceful fallback 필수. 페이지 fetch는 사용자 탭 컨텍스트에서 실행이 안전(anti-bot 회피) — 네이버 **일반 페이지**(스토어 등)는 background 직접 fetch가 490으로 차단돼 MAIN world 대행이 필요하고 탭 navigation은 예외 (형제 프로젝트 실측, `reference_naver_tag_picker` 메모리).
- **cross-account의 silver bullet = `x-ad-customer-id: {masterCustomerId}` 헤더.** `/apis/sa/api/*`는 URL에 계정이 없어도 이 헤더 기준으로 응답. 헤더 없으면 세션 활성 계정 기준 → 404 또는 (stats는) **200+빈 data(silent-empty)**. `masterCustomerId`는 `GET /apis/ad-account/v2/adAccounts/{adAccountNo}` 응답의 `adAccount.masterCustomerId` — 광고관리자 URL의 `adAccountNo`는 검색광고 API `customerId`와 별개다. bizmoney만 예외로 URL-aware(`/apis/bmgate/...`). 새 internal API 도입 시 SPA가 이 헤더를 보내는지 정찰 권장. **단 쓰기(POST)는 규칙이 정반대 — `customerId`를 본문 안에 넣는다.** 빼면 `404 code 1018 "리소스에 접근할 권한이 없습니다"`가 와서 권한 문제로 오진하기 쉽다(2026-08-14 F-AutoSetup 정찰).
...
- **확장 reload 직후엔 크롬이 페이지 로드에 콘텐츠 스크립트를 못 넣는 공백이 있다** — background `onInstalled`가 열린 탭에 재주입(`reinjectContentScripts`, `scripting` 권한). 재주입 시 이전 고아 컨텍스트의 루프와 DOM 싸움 방지는 `src/shared/takeover.ts` 세대 표식(`claimTakeover`/`isStale`) — 새 주기 루프(interval·observer)를 만들면 isStale 은퇴 체크를 넣을 것 (2026-07-22). **중복 초기화 가드를 `window.__dvadsXxxInit = true` 같은 boolean으로 두지 말 것** — window는 격리 세계라 재주입된 새 컨텍스트와 공유돼 새 컨텍스트가 "이미 초기화됨"으로 오판하고 그냥 반환한다. 그 사이 옛 컨텍스트는 takeover에 밀려 UI를 내리고 은퇴하므로 그 기능만 통째로 사라지고 새로고침해야 돌아온다(2026-08-20 다계정 버튼 실사고). 가드는 `currentGen()` 값 비교로.
