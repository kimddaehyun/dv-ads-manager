...
| 위치 | 책임 |
|---|---|
| **`src/lib/product-page-extract.ts`** (신규) | 입력 문자열(URL 또는 ID) → 후보 이미지 URL 배열. 순수 함수, DOM 의존 없음. ads.naver.com 콘텐츠 스크립트뿐 아니라 추후 background에서도 호출 가능. |
| **`src/content/asset-bulk-popup.ts`** (수정) | `buildImageSection` 폐기 → `buildImageBlock` 신규. `ImageSlotInput` 폐기 → `AssetBulkImagesInput` 단일 객체 구조. |
| **`src/content/asset-bulk.ts`** (수정) | `resolveImageFiles` 새 입력 구조 대응 (`files[]` + `selectedUrls[]`). |
| **`src/content/dom-asset.ts`** (수정) | `scanExistingAssets`가 `imageCount`도 산출 — td[4] 유형이 "파워링크 이미지"인 행 카운트. `ExistingAssets`에 `imageCount: number` 필드 추가. |
...
## `src/lib/product-page-extract.ts` 상세
