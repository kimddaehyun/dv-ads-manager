...
- ~~**Task 013: F002 쇼핑검색광고 그룹 inline 펼침 구현**~~ — **보류** (2026-05-19)
  ...
  - **다음 방향**: AE 광고주 보고 카톡 분석(2026-05-19) 결과 진짜 페인은 "ROI 임계값 기반 자동 분류 + 일괄 입찰 변경" + "키워드 패턴 일괄 처리" + "보고서 자동 생성". 다음 brainstorming에서 새 기능 ID로 재정의 예정. → **F-Brief(Task 021)로 실현됨** (2026-07-16) — ROI 임계값 자동 분류(roasBand 3구간)와 보고서 자동 생성(보고 문구+색칠 표)이 여기서 구현. 일괄 입찰 변경은 F-Brief 2단계(설계 §15)로 확정.
...
- **Task 020: F-Report — 광고주 보고용 리포트 엑셀** ✅ - 완료 (2026-05~07 누적, 로드맵 기록 누락분 소급 등재)
  - F-MultiAccount 행 메뉴 "리포트 생성"(단일/일괄 zip) → 기간 선택 → advanced-report 실수집 → 엑셀 양식 주입 → 다운로드. 전부 클라이언트 사이드.
  - 키워드 서버필터(유형+광고비>0+노출>0, 2026-07-16), 그래프 콤보 통일, GFA(디스플레이) 종합 연결, 브랜드검색 계약 일할 계산 포함.
  - 2026-07-16: `collectReportData()` 추출로 수집(2~5단계)과 엑셀 생성(1·6단계) 분리 — F-Brief와 수집 공유, 병렬 구조(성능 감사 2026-07-02) 보존.

- **Task 021: F-Brief — 광고주 보고 문구 생성 (AX 1호)** 🟢 - v1 구현 완료 (2026-07-16), 수동 QA 진행 중
  - 계획: `docs/superpowers/plans/2026-07-16-f-brief.md` / 설계: `docs/superpowers/specs/2026-07-16-f-brief-design.md`
  - ✅ Task 1~8: vitest 도입 + roasBand 3구간(green/yellow/none, 노랑 하한=목표x0.75) + 후보 5종(전환0 키워드/목표미달/지면 전환0/순위 상승 여지/전기 대비 전환 빠진 상품) + canvas 색칠 표 PNG 클립보드 + 패널(블록별 복사) + 계정별 목표 ROAS(`targetRoas`) + 순위 보강(등록 키워드 입찰가 맵 + GET_BID_ESTIMATE)
  - ✅ Task 9~10: Supabase Edge Function `brief-compose`(dvcompany 조직, Gemini 3.1 Flash-Lite — 설계의 2.5 Flash-Lite가 신규 키 제공 중단이라 교체) 배포·검증 + AI 조립 연결(완전자동 + "직접 고르기" AE선택 모드) + 숫자 검산(차단 않고 배지) + AI 판단 문단 좌측 주황 선 + 옵션 "보고 문구 이용 코드"
  - 자동 테스트 39개 통과(vitest — 순수 로직 전용). codex 리뷰 GATE PASS(P2 4건 중 2건 수정·2건 근거 기각)
