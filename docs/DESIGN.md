...
다이얼로그/패널 제목에서 **대상 이름(계정명·광고주명·키워드 등)만 DV 주황으로 강조**한다.
제목 형식은 `동작/문서명 (대상명)` — 예: "리포트 문구 <span class=orange>(고려은단헬스케어)</span>".

```
제목 텍스트: .dvads-confirm-title 등 기존 제목 클래스 그대로
대상명 span: .dvads-title-accent  →  color: #E6783B (굵기는 제목 상속)
```

- 새 다이얼로그를 만들 때 제목에 대상 이름이 들어가면 이 패턴을 쓴다 (색 하드코딩 금지).
- 주황은 화면당 1~2개 원칙에 포함해 계산 — 제목 강조 + Primary 버튼 조합까지는 허용.
...
**다이얼로그 푸터 버튼은 대형 통일 (2026-07-21)** — F-Brief 하단 버튼 비율을 표준으로,
backdrop 다이얼로그 푸터의 버튼은 `flex: 1`(풀폭 양분) + 높이 44px + 14px/600.
기본 `.dvads-btn`(32px)은 인라인 컨텍스트용으로 유지하고, `overlay.css` 말미의
"다이얼로그 푸터 대형 버튼 통일" 블록이 푸터 컨테이너별로 덮는다. 푸터에 버튼 외
요소(토글 등)가 있으면 그 요소만 고정폭. 새 다이얼로그 푸터도 이 블록에 셀렉터를 추가한다.
