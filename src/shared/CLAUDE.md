...
- `auth-gate.ts` — `requireApproved()`가 콘텐츠 스크립트의 단일 관문. **페이지당 1회만 조회하도록 모듈 스코프에 Promise 캐시**(여러 기능이 각자 init에서 부르면 중복 네트워크 호출) — per-page memo라 페이지 재로드 전까지 상태 안 바뀜. **이관은 여기서 fire-and-forget으로 시작되고 `refreshFromServer`는 이관 완료 전엔 가드로 빈손 반환** — 설치 직후 첫 화면이 빈 목록으로 뜨는 원인. 첫 렌더를 이관 뒤로 미루려면 `migrationSettled()`를 기다릴 것(multi-account openPopover 참조, 2026-08-12).
- `server-store.ts` — `account_meta`/`account_groups`/`change_watch_state`(계정 이슈 이력)/`user_settings`(사용자 설정 4종: 알림 제외 변경자·대행권 기준 번호·광고 유형 필터·리포트 담당자명) CRUD. 뒤 둘은 2026-07-20 추가. **`user_settings`는 부분 갱신**(`pushUserSettings(patch)`) — 전체 upsert하면 안 넘긴 설정이 기본값으로 밀린다. 서버에 행이 없으면(도입 전 사용자) 첫 새로고침이 **로컬값을 올려** 리셋처럼 보이는 걸 막는다. **저장은 항상 서버 먼저** — 성공 후 로컬 캐시 갱신은 호출부 책임. `pushGroups`는 전체 교체(replace-all, `not in (ids)` 삭제 + upsert) 전략. **PostgREST는 WHERE 없는 DELETE를 400(`DELETE requires a WHERE clause`)으로 거부** — delete에는 항상 `.eq("user_id", ...)`류 필터를 붙일 것. 그룹 0개 때 조건 없는 delete가 나가 이관이 영구 실패하고, 이관 미완료 → `refreshFromServer` 가드에 막혀 새 설치에서 서버 데이터가 영영 안 내려오던 실사고(2026-08-12). 같은 이유로 migrate-local upload는 로컬 그룹 0개면 pushGroups를 건너뛴다(빈 로컬로 서버 그룹을 지우지 않기 위해).
...
- **ads.naver.com 위 다이얼로그에서 마우스 휠이 안 먹으면 호스트 페이지의 문서 레벨 휠 가로채기가 원인** — 컨테이너 CSS로는 못 풀고, 카드에 `wheel`(passive:false) 핸들러를 달아 `scrollTop`을 직접 움직인다. 내부 스크롤 요소(textarea 등)가 소비 가능하면 그쪽 먼저, 끝에 닿으면 본문으로. shift+휠/가로 제스처는 통과 (`brief-panel.ts` 카드 휠 핸들러가 레퍼런스).
- `tooltip.ts`의 placement는 `top`/`bottom`/`right`(우측, 공간 부족 시 좌측 flip). `attachActionMenu` 항목에 `title`을 주면 native title이 아니라 이 다크 툴팁이 우측에 뜬다. **툴팁은 전역 단일 element라 anchor가 DOM에서 detach되면 mouseleave가 안 와 고아로 남는다** — 호버 중이던 element를 제거/재렌더하는 코드는 `hideTooltip()`을 직접 호출할 것 (ui-dropdown `populate()`가 레퍼런스).
- **토스트·다이얼로그 등 최상위 오버레이는 전부 z-index max(2147483647)라 DOM 순서가 승부를 가른다** — 나중에 body에 붙은 쪽이 위. 토스트는 `ensureRoot`가 띄울 때마다 root를 body 끝으로 옮겨 다이얼로그 위에 오게 한다(toast.ts). 새 최상위 오버레이도 가림 문제가 생기면 z-index 경쟁 대신 이 재부착 패턴을 쓸 것.
