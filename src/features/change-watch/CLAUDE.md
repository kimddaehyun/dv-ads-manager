...
`POST /apis/sa/api/histories/_search?serviceId=james-rhodes&since={ms}&until={ms}&maxRowsPerPage=5000` + body `{bool:{must:[]}}`(ES DSL) + **`x-ad-customer-id` 헤더 필수**(없으면 `ownerId format is invalid`). 5,000행은 호출당 한도일 뿐 — **`fetchChangeHistoryAll`(한도에 걸리면 기간 반분 재귀 + eventId 중복 제거)만 쓸 것.** 맨 `fetchChangeHistory`는 넘친 만큼 조용히 버리는데 조회는 성공 처리라 `scanned_until`이 전진해 그 구간이 영영 사라진다(2026-08-20 라이브에서 실제 발생). 변경자 칩 수집처럼 전수가 필요 없는 곳만 예외. UI(다이얼로그·행 알림)는 `@/features/multi-account/multi-account.ts`에 있고, 이 폴더는 수집·판정 로직(`change-watch.ts`).

## 동작 규칙

- 두 가지를 잡는다: ① 예산 초과 중단 = **`ncc.charge.*_LOCK` 전부**(이름 나열 금지 — 캠페인/광고그룹 하루예산, 공유예산, 앞으로 늘 종류까지 규칙으로 잡는다. `_UNLOCK`(재개)과 `ACCOUNT_LOCK`(비즈머니 소진, 비즈머니 알림 담당)만 제외. `lockLabelOf`) ② 외부 수정 = `ncc.heroes.*`인데 `actorDisplayName`이 제외 목록(`change_watch_identity`)에 없는 경우.
- **종류별 on/off는 계정 설정에서 온다**(`classifyHistory(rows, actors, {budget, external})`). 예산 도달은 **기본 켜짐**(`MultiAccountUserMeta.budgetWatch`가 `false`인 계정만 제외 — `isBudgetWatchOn`), 외부 수정은 **켠 계정만**(`changeWatch`). 조회는 계정당 1회로 같고 걸러내기만 다르다.
...
- 설정은 ⋮ > **"계정 알림 설정"** 하나(`openAccountAlertsDialogFor`) — 비즈머니 부족·브랜드검색 계약 만료·예산 도달·변경 이력을 리포트 설정과 같은 행(`dvads-rdp-set-row`)으로 모아 놓았다(2026-08-20 통합, 이전엔 알림마다 별도 다이얼로그). 제외 변경자 칩은 **변경 이력을 켰을 때만** 지연 로드한다(계정마다 14일치 이력 조회라 비싸다). 제외 목록 자체는 계정별이 아니라 전역이다(설명 라벨은 달지 않는다 — 입력창 안내로 충분).
- **다중 선택은 건드린 항목만 저장한다**(`touched()`) — 값이 계정마다 다른데 통째로 덮으면 안 만진 설정이 지워진다(예산만 켜러 열었는데 비즈머니 기준 증발).
- **설정 확인 시 점검 기록을 지우지 말 것.** 지우면 다음 조회 시작점이 "지금"으로 리셋돼 그 전에 쌓인 예산 도달이 영구히 사라진다 — `clearChangeWatchStates` + "첫 점검 소급 없음"(`eb9accf`) 조합이 만든 실사고(2026-08-20 문의: 8/11~8/19 예산 도달 9건이 통째로 증발, 화면은 "이상 없음"). 지금은 `refilterChangeWatchStates`가 저장된 알림만 새 설정으로 다시 거른다.
- **한 번도 점검 못 한 계정에 "이상 없음" 배지를 달지 말 것**(`syncIssueChip`의 `statusChangeScanned`). 확인도 안 하고 괜찮다고 말하면 위 사고처럼 사용자가 정상으로 오해한다.
- **상태는 Supabase `change_watch_state` 테이블이 원본, 로컬 `change_watch_state:<no>`는 캐시**(2026-07-20 이전). 단 수집 결과라 서버 쓰기 실패는 warn 후 로컬만 반영 — 점검이 끊기면 알림 자체가 죽는다. 내려받기(`refreshFromServer` → `mergeChangeWatchFromServer`)는 **덮어쓰기 금지, id 합집합 + 확인/점검 시각 max 병합** (아직 못 올린 로컬 알림 보호). **로컬 전용 필드(`server_synced_until`)도 병합에서 반드시 보존** — 빠뜨리면 popover를 열 때마다 0으로 떨어져 서버 쓰기 스로틀이 무력화된다(2026-07-22 codex 리뷰 적발). 조회 창은 고정 기간이 아니라 `scanned_until`(직전 점검 시각) 이후 증분 — 누락도 중복도 없음.
...
- 알람/알림 권한 없이 콘텐츠 스크립트가 10분 주기 + popover 진입 시 점검(새 알림 없으면 서버 쓰기 생략 — 단 서버 체크포인트가 1시간 이상 뒤처지면 올려서 로컬 유실 시 공백을 1시간 이내로 제한, `server_synced_until`) — 탭 닫으면 멈추지만 `scanned_until` 덕에 다시 열 때 이어받음. 정찰 결과는 메모리 `project_f_changewatch_endpoints`.
