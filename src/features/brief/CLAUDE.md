...
- **순위 조회(fillRanks)는 선택 화면을 막지 않는다**(2026-07-22 속도 개선) — 화면을 먼저 띄우고 완료 시 `mergeLateRankCandidates`가 후보를 늦게 합류. 핸들은 `ctx.liveHandle`(showSelection이 매번 갱신) — 렌더 시점 핸들을 캡처해 쓰면 이슈 기준 변경 후 순위 후보가 조용히 사라진다(코덱스 P2). 체크 보존은 `candKey`(kind+scope+facts) 인덱스 매핑. `renderBriefPickPanel`은 `BriefPickHandle{isLive,snapshot}` 반환.
- **`warmCompose()`** — 선택 화면 진입 시 서버 `mode:"warmup"`(AI 미호출, 인증 후 즉시 200)으로 콜드스타트를 사용자 대기에 흡수. 60초 쓰로틀(서버가 인증 검증은 매번 하므로). warmup 분기는 서버 배포 필요 — 미배포 서버에 보내면 blocks 모드로 흘러 빈 생성 1회가 나간다.
