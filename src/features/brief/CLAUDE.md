...
F-MultiAccount 행 메뉴 "보고 문구" → 기간 선택 → 수집(성과 ∥ 지난 보고 ∥ 변경 이력) → 규칙 엔진 후보 추출 → **이슈 선택 화면 먼저**(캠페인 > 그룹 계층 체크 + 고급 옵션: 보고 유형 칩/이슈 기준/대화 스타일, 2026-07-21 개편) → 선택한 이슈만 AI 조립 → 결과 패널(재생성 버튼군 + 저장/복사). **자동 전체 생성 모드는 폐기**(2026-07-19 구조 개편) — AE가 고른 것만 문구가 된다. 설계 `docs/superpowers/specs/2026-07-16-f-brief-design.md` / 계획 `docs/superpowers/plans/2026-07-16-f-brief.md`.
...
- 상품 후보(`productConvDrop`)는 **매출 감소율이 광고비 감소율보다 클 때만** — 광고비를 줄여 매출이 따라 준 경우는 성과 이슈 아님(2026-07-21).
...
- 선택 화면: 캠페인/그룹 머리글 클릭=자식 일괄 선택, 호버=자식 하이라이트, 머리글 **제목 글자** 클릭=광고관리자 새 탭(캠페인 `/sa/campaigns/{id}`, 그룹 `/sa/adgroups/{id}`). 전체 "모두 선택"·이력 토글은 폐기.
- 고급 옵션: 보고 유형·이슈 기준=칩 + i 툴팁(클릭), 대화 스타일은 인라인(화살표로 생성, 생성/수정 시 자동 저장 - 저장 버튼 없음). `brief-tone-panel.ts`는 데드코드(미참조, 삭제 예정). "지난 보고 보기" 버튼은 내림 — 저장(brief-history)은 유지.
