...
- **캠페인 상세 SPA URL은 미확인** — 추정 `/sa/campaigns/{id}`는 404(2026-07-20). 캠페인으로 보낼 땐 검증된 `campaigns-by/{TYPE}` 목록으로. API campaignType ↔ SPA TYPE이 다름: `SHOPPING`↔`SHOPPING_NS`, `BRAND_SEARCH`↔`BRAND` (`ISSUE_DEST_CAMPAIGN_TYPES` 매핑 참조). 광고그룹 상세 `/sa/adgroups/{id}`는 검증됨.
