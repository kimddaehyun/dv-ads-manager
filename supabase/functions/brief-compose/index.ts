...
Deno.serve(async (req) => {
  ...
  if (body.mode === "reportSummary") {
    ...
    const summaryPrompt = [
      ...
      "- 두 기간 모두 전환이 없는 키워드는 노출 제외나 저효율 광고그룹으로 분리하는 방향을 써라. 이번 기간에만 전환이 없는 키워드는 아직 바꾸지 말고 유지하면서 지켜보는 방향을 써라.",
      ...
      "- [키워드 목록 - 정보성 판별용]에서 구매 의도가 아니라 정보를 찾는 검색어(추천, 가격, 후기, 방법, 효과, 효능, 사용법, 착용법 같은 말이 붙은 것)가 눈에 띄면 한 문단으로 묶어 다뤄라. 눈에 띄지 않으면 그 문단은 넣지 마라. 이름을 하나하나 나열하지 말고 공통으로 들어간 말로 묶어 불러라. 예: [후기]가 포함된 키워드, [가격] 관련 키워드.",
      "- 정보성 검색어는 정보 취득이 목적이라 구매로 이어지는 경우가 드물다. 그래서 조치가 셋으로 갈린다: (가) 전환이 없으면 입찰가 하향이 아니라 제외를 제안해라 - 입찰가를 낮춰도 성과가 잘 바뀌지 않는 영역이다. (나) 전환은 있으나 ROAS가 계정 전체 ROAS에 크게 못 미치면 역시 제외를 제안해라. (다) ROAS가 계정 전체 수준 이상이면 제외하지 말고 입찰가만 낮추는 방향으로 써라 - (다)에는 제외하겠다는 말을 쓰지 마라.",
