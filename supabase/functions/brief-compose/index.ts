...
Deno.serve(async (req) => {
  ...
  if (body.mode === "reportSummary") {
    ...
    // 이전 기간 비교가 가능하면 클라이언트가 lowKeywordLines 대신 아래 둘로 나눠 보낸다.
    const lowKeywordBothLines = Array.isArray(body.lowKeywordBothLines) ? body.lowKeywordBothLines.slice(0, 5) : [];
    const lowKeywordRecentLines = Array.isArray(body.lowKeywordRecentLines) ? body.lowKeywordRecentLines.slice(0, 5) : [];
    const lowSubGroupLines = Array.isArray(body.lowSubGroupLines) ? body.lowSubGroupLines.slice(0, 5) : [];
    const infoCandidateLines = Array.isArray(body.infoCandidateLines) ? body.infoCandidateLines.slice(0, 50) : [];
    const genderLines = Array.isArray(body.genderLines) ? body.genderLines.slice(0, 2) : [];
    // 광고주 목표 ROAS. 빈 문자열이면 미설정 — 부진 판정이 전환 0건만 본 것이라 목표를 언급하면 안 된다.
    const targetRoasText = typeof body.targetRoasText === "string" ? body.targetRoasText : "";
    ...
    const summaryPrompt = [
      ...
      "- 전환수: (데이터 그대로)",
      "",
      "(분석 코멘트 - 2~5개 문단. 잘된 부분과 개선할 부분, 조치를 담되 순서와 문단 구성은 데이터에 맞게 자유롭게. 문단 사이에 빈 줄)",
      "",
      ...
      "  예시2) [OO] 키워드는 광고비 3만 원에 매출 90만 원이 발생하여 효율이 준수합니다. 반면 [XX] 키워드는 이번 기간에 전환이 없었으나 이전 기간에 전환 3건, 매출 40만 원이 발생한 키워드로, 현재 입찰가를 유지하고 확인해 보겠습니다.",
      ...
      "- ROAS 낮을 때 다음 액션 예시: 입찰가 하향 조정, 소재 문구 수정, 확장소재 추가, 조정 후 데이터 확인 등. 제외나 별도 분리는 아래 매체 구분 규칙을 따를 것",
      ...
      "- 저효율 키워드는 키워드마다 문장을 따로 쓰지 말고 목록 단위로 묶어 한 문단에 담아라. 문단 안에 이름은 대괄호로 2~3개까지만 들고 나머지는 생략해라.",
      "- 두 기간 모두 부진한 키워드는 노출 제외나 저효율 광고그룹으로 분리하는 방향을 써라.",
      "- 이번 기간에만 부진한 키워드는 이번 기간 상황을 먼저 말하고, 이전 기간 성과를 숫자와 함께 유지 근거로 뒤에 붙여라. 흐름은 이렇다: 이번 기간에 전환이 없었으나 이전 기간에 전환 N건이 발생한 키워드로, 현재 입찰가를 유지하고 확인해 보겠습니다. 전환은 있는데 수익률이 목표에 못 미친 경우라면 '전환이 없었으나' 자리에 '수익률이 목표에 못 미쳤으나'를 써라. 숫자는 목록의 '이전 기간' 뒤에 있는 값을 그대로 써라.",
      "- 다만 목록에 '집행이 적어 비교 어려움' 또는 '집행 없음'이라고 적힌 키워드는 이전 기간 성과를 유지 근거로 삼지 마라. 이전에 성과가 있었다고 말하면 사실과 다르다. 이번 기간 상황만 말하고 지켜보겠다고 써라.",
      "- 그 문단에 '한 기간만으로 판단하기 이르다' 같은 설명을 덧붙이지 마라. 유지하겠다는 말 자체로 충분하다. 이 목록에는 입찰가 하향이나 제외를 쓰지 마라.",
      "- [저효율 - 파워링크와 쇼핑검색 광고그룹]은 주력 매체라 광고를 내리기 어렵다. 이 목록에는 입찰가를 조정하는 방향으로만 써라. 제외하거나 별도 그룹으로 분리하겠다는 조치를 쓰지 마라.",
      "- [저효율 - 그 밖의 매체 광고그룹]은 데이터를 보고 노출 제외 또는 별도 그룹 분리 중 맞다고 판단되는 쪽을 골라 써라.",
      "- [키워드 목록 - 정보성 판별용]에서 정보를 찾는 검색어를 골라라. 판별 기준은 딱 하나다 - 키워드 이름 안에 정보를 찾는 말이 실제로 들어 있는가. 추천, 가격, 후기, 방법, 효과, 효능, 성분, 사용법, 착용법, 비교, 순위, 부작용, 차이 같은 말이다. 상품명이나 브랜드명만 있는 키워드는 정보성이 아니다.",
      "- 성과가 나쁘다는 이유로 정보성이라고 부르지 마라. 전환이 없다는 것과 정보를 찾는 검색어라는 것은 별개다. 위 기준에 맞는 키워드가 목록에 없으면 정보성 문단을 통째로 빼라. 억지로 만들지 마라.",
      "- 정보성 키워드는 이름을 하나하나 나열하지 말고 공통으로 들어간 말로 묶어 불러라. 예: [후기]가 포함된 키워드, [가격] 관련 키워드.",
      "- 정보성 검색어는 정보 취득이 목적이라 구매로 이어지는 경우가 드물다. 그래서 조치가 셋으로 갈린다: (가) 전환이 없으면 입찰가 하향이 아니라 제외를 제안해라 - 입찰가를 낮춰도 성과가 잘 바뀌지 않는 영역이다. (나) 전환은 있으나 ROAS가 기준에 크게 못 미치면 역시 제외를 제안해라. 기준은 [광고주 목표 ROAS]가 있으면 그 값, 없으면 계정 전체 ROAS다. (다) ROAS가 그 기준 이상이면 제외하지 말고 입찰가만 낮추는 방향으로 써라 - (다)에는 제외하겠다는 말을 쓰지 마라.",
      "- 같은 키워드가 정보성으로도 보이고 저효율 키워드 목록(두 기간 모두든 이번 기간만이든)에도 있으면 정보성 판단을 우선해라. 그 키워드는 저효율 문단에서 빼고 정보성 문단에서만 다뤄라. 한 키워드를 두 문단에서 다루거나 서로 다른 조치를 동시에 말하지 마라.",
      "- 정보를 찾는 검색어는 기간을 더 두고 봐도 전환이 잘 나지 않는 영역이다. 그래서 이번 기간에만 부진해도 지켜보는 의미가 적고, 저효율 광고그룹으로 분리해 입찰가를 낮추는 것도 효과가 적다. 정보성으로 판단했으면 조치는 제외 아니면 입찰가 하향 둘 중 하나로만 가라.",
      "- 정보성 검색어 제외를 제안했으면 광고주가 남길 것을 고를 수 있게 한 문장을 덧붙여라. 예: 유지를 원하시는 키워드가 있으면 말씀 부탁드립니다.",
      "- 여러 키워드의 금액을 더한 합계 숫자를 지어내지 마라. 준 숫자만 그대로 쓰고, 금액이 쌓인다는 점은 숫자 없이 표현해라. 예: 키워드마다 금액은 크지 않으나 모이면 적지 않은 비용입니다.",
      "- [성별 성과]에 값이 있으면 성별 간 성과 차이를 짚고, 효율이 좋은 성별의 입찰 가중치를 올리고 낮은 쪽을 내리는 방향을 제안해라. (없음)이면 성별은 언급하지 마라.",
      ...
      "- 분석 코멘트는 최대 5문단 - 길게 늘어놓지 마라. 재료가 없는 주제는 문단을 만들지 마라",
      "- 불확실한 추측 금지",
      "- 'AI가 분석했습니다' 같은 표현 금지",
      "",
      "[데이터]",
      ...
      "",
      ...(targetRoasText ? [`[광고주 목표 ROAS] ${targetRoasText} - 아래 저효율 목록은 전환이 없거나 이 목표에 못 미치는 것들이다`, ""] : []),
      "[저효율 - 성과가 부진한 키워드]",
      ...(lowKeywordLines.length > 0 ? lowKeywordLines : ["(없음)"]),
      "",
      "[저효율 - 두 기간 모두 부진한 키워드]",
      ...(lowKeywordBothLines.length > 0 ? lowKeywordBothLines : ["(없음)"]),
      "",
      "[저효율 - 이번 기간에만 부진한 키워드]",
      ...(lowKeywordRecentLines.length > 0 ? lowKeywordRecentLines : ["(없음)"]),
      "",
      "[저효율 - 파워링크와 쇼핑검색 광고그룹]",
      ...(lowGroupLines.length > 0 ? lowGroupLines : ["(없음)"]),
      "",
      "[저효율 - 그 밖의 매체 광고그룹]",
      ...(lowSubGroupLines.length > 0 ? lowSubGroupLines : ["(없음)"]),
      "",
      "[키워드 목록 - 정보성 판별용]",
      ...(infoCandidateLines.length > 0 ? infoCandidateLines : ["(없음)"]),
      "",
      "[성별 성과]",
      ...(genderLines.length > 0 ? genderLines : ["(없음)"]),
      "",
      "숫자는 제공된 데이터를 정확히 사용해. 임의로 반올림하거나 변경하지 마.",
      "설명이나 머리말, ===템플릿=== 표시 없이 카톡에 그대로 붙여넣을 문구만 출력해.",
      "줄바꿈과 빈 줄을 템플릿대로 반드시 유지해 - 한 덩어리로 이어 붙이지 마.",
      "마지막 점검 1: 뒷받침/견인/도모/제고/기여 같은 단어가 한 번이라도 들어갔으면 쉬운 말로 바꾼 뒤 출력해라.",
      "마지막 점검 2: 같은 키워드 이름이 두 문단에 나오면 한 곳만 남기고 다른 곳에서는 지워라. 한 키워드에 서로 다른 조치(유지와 제외, 입찰가 하향과 제외 등)를 동시에 말하면 안 된다.",
      "마지막 점검 3: 정보성이라고 부른 키워드에 추천/가격/후기/방법/효과/효능/성분/사용법/착용법/비교/순위/부작용/차이 같은 말이 실제로 들어 있는지 확인해라. 하나도 없으면 그 문단을 지우고 출력해라.",
    ].join("\n");
    const url = `https://generativelanguage.googleapis.com/v1beta/models/${MODEL}:generateContent?key=${API_KEY}`;
